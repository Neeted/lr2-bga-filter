...
2. **LAV Video Decoder の設定**
    ...
    ![LAV Video Decoder 推奨設定](docs/img/lav_setting.png)

    - **出力色空間の制限 (Output Formats)**: 上図の通り、**RGB32 のみをチェックし、他をすべて外してください。**
        - *理由*: LR2 本体は RGB24 入力しか受け付けません。LAV の出力を RGB32 に限定することで、「LAV (RGB32) -> [本フィルター] -> LR2 (RGB24)」という経路を意図的に成立させ、本フィルターが強制的に使用されるようにします。
    - **ハードウェアデコーダーの有効化 (Hardware Acceleration)**: 上図の通り、Hardware Decoder を **D3D11** に設定することを推奨します。
        - 高解像度・高フレームレートのBGA（例: 2000x1500 60fps AVC）をソフトウェアデコードで再生すると、CPUへの負荷が高まり、ノーツ描画の滑らかさに影響が出る場合があります。ハードウェアデコーダーを有効にすることで、動画デコード処理がGPUにオフロードされ、CPU負荷が軽減されます。
        - ほとんどの環境では D3D11 を選択すれば問題ありません。GPU が対応していない場合はデフォルトの None のままでも動作しますが、高負荷なBGAでパフォーマンスが低下する可能性があります。
