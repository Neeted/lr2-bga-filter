...
- [ ] Only output to LR2: フィルターの接続元プロセス名に「body」が含まれる場合のみ接続を許可します。デフォルトで有効です。これを無効にすると、GraphStudioNextなどの外部ツールからフィルターを使用できるようになりますが、予期しない動作を引き起こす可能性があります。
- [ ] Only output to renderer: フィルターの出力先がレンダラー（出力ピンを持たないフィルター）の場合のみ接続を許可します。デフォルトで有効です。これを無効にすると、後段に別のフィルターを接続できるようになります。
