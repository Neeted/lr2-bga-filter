...
- [ ] Only output to LR2: 필터의 접속원 프로세스명에 「body」가 포함되는 경우만 접속을 허가합니다. 기본값으로 유효합니다. 이것을 무효로 하면, GraphStudioNext 등의 외부 툴로부터 필터를 사용할 수 있게 됩니다만, 예기치 않은 동작을 일으킬 가능성이 있습니다.
- [ ] Only output to renderer: 필터의 출력처가 렌더러(출력 핀을 가지지 않는 필터)인 경우만 접속을 허가합니다. 기본값으로 유효합니다. 이것을 무효로 하면, 후단에 다른 필터를 접속할 수 있게 됩니다.
