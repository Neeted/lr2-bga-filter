...
LR2 (Lunatic Rave 2) 用の DirectShow フィルターです。
BGA の再生品質とパフォーマンスを改善することを目的としています。
...
3.  **メリット値 (優先度) の設定**
    - 本フィルター (`LR2BGAFilter`) を **最も高い優先度 (Merit Value)** に設定してください。
...
### 注意事項 (免責)
上記の設定（LAVの出力制限やメリット値の変更）は、DirectShowを使用するシステム全体（他の動画プレイヤーなど）に影響を与える可能性があります。
